...
function pagetest($child) {
  global $o, $cron;
  $event_date = strtotime( substr($child->path->path,17,10) );
  if ($cron) {
    $o[3] .= '<h4>'.$child->path->path.'</h4>';
  }
  if (preg_match('/[0-9]\/20/', $child->path->path) && $event_date < time()-48*3600 ) {
    return true;
  } else {
    if (!$cron) {
      echo $child->path->path.'<br/>';
      // echo substr($child->path->path,17,10);
      // echo '<br/>'.$event_date.'<br/>';
      // echo time()-48*3600;
      // echo '<br/>';
    }
  }
