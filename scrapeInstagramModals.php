...
foreach ($media as $key => $value) {
	...
	if ($key < 4 ) {
		...
		if ($key < 4 ) {
			...
			$output .= '				<img class="lazy" data-original="/_assets/instagram/large/'.$account.'-'.$value->code.'.jpg'.'" src="/_assets/images/loading-ball.gif" width="'.$value->dimensions->width.'" height="'.$value->dimensions->height.'" alt="'.str_replace('"','',$value->caption).'"/>'."\n";
		}
