...
// $start_asset = '52bcf8e07f000002001344a89985168b,c683fba97f00000201c142c2f32ef7d9,c684378c7f00000201c142c2cbd1b16d'; /* (News, not archived) */
// $start_asset = '03ead72b7f000002001344a87a1d76ac'; /* Archinved News */
// $start_asset = 'ab880f697f0000021a23b0063cc5fd6f'; /* (mag) */
...
// $start_asset = 'dde177857f00000246be153906311094,ddf0d5a37f00000246be153957629cc1,ddf218917f00000246be15396b4f1ac2,ddf2c1847f00000246be153972e0fa42,ddf373da7f00000246be15390c07ceaa,ddf420907f00000246be15396893c2f3,ddf7c98e7f00000246be153952cda667,ddf70d177f00000246be1539932df4b1,d8d055c57f00000253b2df1311c74b73'; /* Grad Programs */
...
function foldertest($child) {
  if (!preg_match('/^_admin/', $child->path->path) && !preg_match('/^media/', $child->path->path) && !preg_match('/^events\/20/', $child->path->path))
    return true;
}
...
function changes(&$asset, $type) {
  ...
  if ($_POST['action'] == 'edit') {
    if ($asset["shouldBePublished"] != 1 && $asset["shouldBeIndexed"] != 1) {
      $myFile = "indexes/none.html";
      $fh = fopen($myFile, 'a') or die("can't open file");
      $str = '<div><a href="'.CMS_OPEN_PATH.$asset['id'].'&type='.$type.'#highlight">'.strtoupper($type).': '.$asset['siteName'].'://'.$asset['path']."</a></div>\n";
      fwrite($fh, $str);
      fclose($fh);
    } else if ($asset["shouldBePublished"] != 1) {
      $myFile = "indexes/pub.html";
      $fh = fopen($myFile, 'a') or die("can't open file");
      $str = '<div><a href="'.CMS_OPEN_PATH.$asset['id'].'&type='.$type.'#highlight">'.strtoupper($type).': '.$asset['siteName'].'://'.$asset['path']."</a></div>\n";
      fwrite($fh, $str);
      fclose($fh);
    } else if ($asset["shouldBeIndexed"] != 1) {
      $myFile = "indexes/ind.html";
      $fh = fopen($myFile, 'a') or die("can't open file");
      $str = '<div><a href="'.CMS_OPEN_PATH.$asset['id'].'&type='.$type.'#highlight">'.strtoupper($type).': '.$asset['siteName'].'://'.$asset['path']."</a></div>\n";
      fwrite($fh, $str);
      fclose($fh);
