...
$title = 'Test the existence of Media files in Cascade that are related to files found on the server';

$lines = file('../indexes/stu-media-list.txt', FILE_IGNORE_NEW_LINES);
...
$ignored = 0;
$inUnpubFolder = 0;
file_put_contents('../indexes/stu-media-unpublishable.txt', "");
file_put_contents('../indexes/stu-media-unpublishable-folder.txt', "");
file_put_contents('../indexes/stu-media-missing.txt', "");

foreach ($lines as $path) {
  $cascadePath = substr($path, 2);

  // if ( preg_match('/^(faculty-bak|media|communications)\//', $cascadePath) ) {
  if ( preg_match('/^about\/visit\/360\//', $cascadePath) ) {
    $ignored++;
    continue;
  }
  ...
  } elseif (preg_match('/^magazine\//', $cascadePath) ) {
    $siteName = 'www.sarahlawrence.edu+magazine';
    $cascadePath = substr($cascadePath, 9);
    echo '<div class="k">Reading '.$cascadePath.' in '.$siteName.'</div>';
  } else {
  ...
  if ($reply->readReturn->success == 'true') {
    ...
    if ( $asset['shouldBePublished'] == 1 ) {
      ...
      foreach($parentFolders as $folder) {
        ...
        if ( in_array($folderPath, $publishableFolders) ) {
          ...
          file_put_contents('../indexes/stu-media-unpublishable-folder.txt', $folderPath.' in '.$cascadePath."\n", FILE_APPEND);
      ...
      file_put_contents('../indexes/stu-media-unpublishable.txt', $cascadePath."\n", FILE_APPEND);
      echo '<div class="f">The '.$cascadePath.' Media is unpublishable, yet is published.</div>';
      $unpub++;
    ...
    file_put_contents('../indexes/stu-media-missing.txt', $cascadePath."\n", FILE_APPEND);
    echo '<div class="f">The '.$cascadePath.' Media in site '.$siteName.' is absent from Cascade.</div>';
    $absent++;
