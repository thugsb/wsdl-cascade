...
$title = 'Test the existence of Documents in Cascade that are related to files found on the server';

$lines = file('../indexes/stu-docs-list.txt', FILE_IGNORE_NEW_LINES);
...
$ignored = 0;
$inUnpubFolder = 0;
file_put_contents('../indexes/stu-docs-unpublishable.txt', "");
file_put_contents('../indexes/stu-docs-unpublishable-folder.txt', "");
file_put_contents('../indexes/stu-docs-missing.txt', "");
...
foreach ($lines as $path) {
  ...
  if ($reply->readReturn->success == 'true') {
    ...
    if ( $asset['shouldBePublished'] == 1 ) {
      ...
      foreach($parentFolders as $folder) {
        ...
        if ( in_array($folderPath, $publishableFolders) ) {
          ...
          file_put_contents('../indexes/stu-docs-unpublishable-folder.txt', $folderPath.' in '.$cascadePath."\n", FILE_APPEND);
      ...
      file_put_contents('../indexes/stu-docs-unpublishable.txt', $cascadePath."\n", FILE_APPEND);
      echo '<div class="f">The '.$cascadePath.' Document is unpublishable, yet is published.</div>';
      $unpub++;
    ...
    file_put_contents('../indexes/stu-docs-missing.txt', $cascadePath."\n", FILE_APPEND);
    echo '<div class="f">The '.$cascadePath.' Document in site '.$siteName.' is absent from Cascade.</div>';
    $absent++;
